...
import {
  loai_nha_dat_ban,
  loai_nha_dat_thue,
} from '../constants/LoaiNhaDat.constant.js';
import homeDirection from '../constants/homeDirection.constant.js';
...
class nhaDatController {
  // @route POST /api/nha-dat/create
  // @desc Tạo tin bds mới
  // @access Private
    ...
    try {
      ...
      if (!files || files.length < 4) {
        return responseHanlder.badrequest(res, {
          err: 'Phải có ít nhất 4 hình ảnh mô tả tin bất động sản!',
        });
      }
  ...
  // @route PUT /api/nha-dat/update/:postId
  // @desc cập nhật tin bds
  // @access Private
  async update(req, res) {
    try {
      ...
      let newSlug = currentDoc.slug; // giá trị mặc định

      if (bodyData.title !== currentDoc.title) {
        newSlug = generateSlug(bodyData.title);

        // kiêm tra slug đã tồn tại chưa
      ...
      // nếu có sữa ảnh thì đâu tiền sẽ kiểm tra là có đủ ảnh không
      ...
      // ở đây xử lý quá trình xóa folder cũ và tạo folder ảnh mới
      let collections = currentDoc.collections;
      if (files.length >= 4) {
        await deleteFolderCollections(currentDoc.id); // xoa folder cu
        collections = await uploadToS3(files, currentDoc.id); // upload ảnh mới
      }
  ...
  // @route DELETE /api/nha-dat/delete/:postId
  // @desc xóa tin bds
  // @access Private
  ...
  // @route GET /api/nha-dat/list/:demand
  // @desc lấy danh sách tin bds bán hoặc thuê hợp lệ(đã được xác nhận)
  // @access Public
  async queryNhaDat(req, res) {
    ...
    // kiểm tra code mã tỉnh, tp nếu có
    if (
  ...
  // @route GET /api/nha-dat/user/list
  // @desc lấy danh sách tin bds của user
  // @access Private
  ...
  // @route GET /api/nha-dat/detail/:slug
  // @desc lấy chi tiết tin bds
  // @access Public
  ...
  // @route GET /api/nha-dat/admin/list/:demand
  // @desc Lấy toàn bộ tin
  // @access Admin
  ...
  // @route GET /api/nha-dat/admin/detail/:slug
  // @desc Lấy chi tiết của bất đống sản
  // @access Admin
  ...
  // @route PUT /api/nha-dat/update-status/:postId
  // @desc change status of ban tin
  // @access Admin
