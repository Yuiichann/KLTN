import brokerFields from '../constants/brokerFields.constant.js';
...
class brokerController {
  // @route POST /api/broker/create
  // @desc 1 user tạo 1 hồ sơ môi giới
  // @access Private
  async create(req, res) {
    try {
      ...
      if (currentUser.isBroker) {
        return responseHandler.badrequest(res, {
          err: 'Người dùng đã đăng ký làm môi giới trước đó.',
        });
      }
      ...
      // khởi tạo trường location và xóa các trường province, district
      fields.map((field) => {
      ...
      // update lại trạng thái broker của document user (không cần đợi)
  ...
  // @route PUT /api/broker/update
  // @desc Chỉnh sữa thông tin môi giới
  // @access Private
  ...
  // @route DELTE /api/broker/delete
  // @desc xóa thông tin môi giới
  // @access Private
  async deleteBroker(req, res) {
    try {
      ...
      userModel.findByIdAndUpdate(user.id, { isBroker: false }).catch(() => {
        return responseHandler.badrequest(res, {
          err: 'Lỗi trong quá trình thay đổi thông tin môi giới.',
        });
      });

      responseHandler.okwithmsg(res, 'Xóa thông tin môi giới thành công!');
    } catch (error) {
      responseHandler.error(res, error);
    }
  }

  // @route GET /api/broker/list (proCode, distCode, fields)
  // @desc lấy danh sách môi giới
  // @access Public
  async getList(req, res) {
    try {
      ...
      if (brokers.length === 0) {
        return responseHandler.notfound(res, {
          err: 'Không tìm thấy nhà môi giới',
        });
      }
  ...
  // @route GET /api/broker/detail/:brokerId
  // @desc lấy chi tiết của 1 môi giới
  // @access Public
  async getDetailBroker(req, res) {
    try {
      ...
      if (!broker) {
        return responseHandler.notfound(res, {
          err: 'Không tìm thấy thông tin nhà môi giới',
        });
      }
  ...
  // @route GET /api/broker/detail-by-user
  // @desc lấy chi tiết môi giới bởi user đã đăng ký môi giới đó
  // @access Private
  ...
  // @route GET /api/broker/top
  // @desc danh sách broker tiêu biểu cho landing page
  // @access Public
