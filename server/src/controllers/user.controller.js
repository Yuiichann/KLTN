...
class userController {
  // @route GET /api/user/pb/:user_id
  // @desc Lấy thông tin công khai của 1 người dùng trên ứng dụng
  // @access Public
  ...
  // @desc Lấy info của user đang đăng nhập
  // @access Private
  ...
  // @route PUT /api/user/update-info
  // @desc Cập nhật các thông tin: displayName, gender, phoneNumber, address
  // @access Private
  async updateInfo(req, res) {
    try {
      const { id } = req.user;
      // const { displayName, address, gender, phone_number } = req.body;

      const updateUser = await userModel
        .findByIdAndUpdate(id, { ...req.body }, { returnDocument: 'after' })
        .select('-password -role -createdAt -updatedAt');

      // udpate broker when uppdate user
  ...
  // @route PUT /api/user/set-avatar
  // @desc Thay đổi avatar người dùng
  // @access Private
  ...
  // @desc Người dùng thực hiện chức năng quên mật khẩu
  // @access Public
  async forgetPassword(req, res) {
    try {
      ...
      // tạo 1 token để xác thực trong 5 phút
      ...
      // Gửi mail về email của tài khoản khôi phục
  ...
  // @route POST /api/user/forget-password
  // @desc Người dụng xác thực thành công và đổi mật khẩu
  // @access Private
  async resetPassword(req, res) {
    try {
      const { password, token, userId } = req.body;

      // bắt lỗi token không hợp lệ ở dây
      try {
        jwt.verify(
          token,
          config.jwt.ACCESS_TOKEN_SECRET,
  ...
  /*
      forgetPassword(1) và resetPassword(2) luôn liên quan tới nhau
      Khi người dùng nhập username hợp lệ ở (1), thì 1 email sẽ được gửi về email đki tài khoản.
      Link trong email đó sẽ điều hướng người dùng đến trang đổi mật khẩu bên FE.
      Trong link đó sẽ chứa token và userId của người dùng.
      Người dùng nhập mật khẩu mới và submit ==> route của (2)
      trong (2), nó sẽ kiểm tra token hợp lệ thì sẽ reset mật khẩu theo mật khẩu người dùng đã đặt.
  */

  // @route GET /api/user/users
  // @desc Lấy danh sách người dùng của ứng dụng
  // @access Admin
  async getUsers(req, res) {
    try {
      ...
      // tìm kiếm theo username or displayname
  ...
  // @route GET /api/user/admin/detail/:userId
  // @desc Lây chi tiết người dùng
  // @access Admin
  ...
  // @route PUT /api/user/lock-user/:userId
  // @desc KhÓa tài khoản user
  // @access Admin
  ...
  // @route PUT /api/user/unlock-user/:userId
